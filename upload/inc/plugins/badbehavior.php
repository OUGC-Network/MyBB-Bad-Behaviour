...
function bb2_read_settings()
{
    ...
    if (strlen($mybb->settings['badbehavior_httpbl_key']) != 12 or !ctype_lower(
...
KEY;
        ...
        if (!$mybb->input['action']) {
            ...
            if ($mybb->request_method == 'post') {
                ...
                if (!$db->fetch_field(
                    $db->simple_select('badbehavior', 'id', 'id=' . intval($mybb->input['id']), array('limit' => 1)
                    ),
                    'id'
                )) {
                ...
                $form = new Form(
                    "index.php?module=tools-badbehavior&amp;action=delete_log&amp;id={$mybb->input['id']}&amp;my_post_key={$mybb->post_code}",
                    'post'
                );
            ...
            if ($mybb->request_method == 'post') {
                ...
                $db->delete_query(
                    'badbehavior',
                    'UNIX_TIMESTAMP(date) < ' . (TIME_NOW - intval($mybb->input['days']) * 60 * 60 * 24)
                );
                ...
                $form = new Form(
                    "index.php?module=tools-badbehavior&amp;action=prune&amp;days={$mybb->input['days']}&amp;my_post_key={$mybb->post_code}",
                    'post'
                );
